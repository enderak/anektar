import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';

const resources = {
  TR: {
    translation: {
      "title": "SAKRAD 3 BOYUTLU ANAHTARLIK ÜRETİCİ",
      "settings_title": "Oluşturucu Ayarları",
      "settings_desc": "3D yazdırılabilir anahtarlığınızı özelleştirin",
      "language": "Dil",
      "label_text": "Ana Metin",
      "sub_text": "Alt Metin (Opsiyonel)",
      "phone_text": "Arka Yüz (Telefon vb.)",
      "placeholder": "Örn: YONCALI",
      "placeholder_sub": "Örn: TA2NLE",
      "placeholder_phone": "Örn: 0555 555 55 55",
      "hole_position": "Delik Konumu",
      "hole_top_left": "Üst Sol",
      "hole_center_left": "Orta Sol",
      "hole_bottom_left": "Alt Sol",
      "hole_top_right": "Üst Sağ",
      "hole_center_right": "Orta Sağ",
      "hole_bottom_right": "Alt Sağ",
      "hole_none": "Deliksiz (Düz)",
      "hole_radius": "Delik Yarıçapı (mm)",
      "hole_thickness": "Halka Et Kalınlığı (mm)",
      "hole_height": "Halka Yüksekliği (mm)",
      "text_style": "Yazı Tarzı",
      "style_embossed": "Kabartma",
      "style_engraved": "Gömülü",
      "style_carved": "Oyuk",
      "warning_stencil_title": "Hassas Baskı Uyarısı:",
      "warning_stencil_desc": "Yazı Derinliği, Taban Yüksekliğine eşit veya büyük. Yazı anahtarlığın içinden tamamen geçerek boşluk oluşturacaktır. Gömülü (oyuk) kalması için taban yüksekliğini artırabilir veya yazı derinliğini azaltabilirsiniz.",
      "base_shape": "Taban Şekli",
      "shape_rectangle": "Dikdörtgen",
      "shape_teardrop": "Damla",
      "shape_heart": "Kalp",
      "shape_contour": "Kontur",
      "font_family": "Yazı Tipi",
      "icon": "Simge",
      "icon_none": "Yok",
      "icon_clover": "Yonca",
      "icon_star_crescent": "Ay Yıldız",
      "icon_heart": "Kalp",
      "icon_skull": "Kurukafa",
      "icon_rook": "Kale",
      "icon_racket_table": "Masa Tenisi Raketi",
      "icon_racket_tennis": "Kort Tenisi Raketi",
      "icon_i_love": "I ❤️ Formatı",
      "icon_custom": "Özel SVG",
      "icon_position": "Simge Konumu",
      "icon_pos_left": "Sol",
      "icon_pos_top": "Üst",
      "icon_pos_right": "Sağ",
      "transform": "Dönüştürme",
      "normal_text": "NORMAL",
      "italic": "İTALİK",
      "text_depth": "Kabartma Yüksekliği (mm)",
      "engrave_depth": "Gömme Derinliği (mm)",
      "phone_depth": "Arka Yüz Oyma (mm)",
      "filament_color": "Filaman Rengi",
      "plate_thickness": "PLAKA KALINLIĞI",
      "tilt_angle": "EĞİM AÇISI",
      "export_btn": "STL DOSYASI OLARAK ÇIKAR",
      "tip": "İpucu: 30-40 derece arası bir eğim (geriye yatıklık), destek (support) kullanmadan kusursuz FDM baskıları almanızı sağlar.",
      "export_ready": "DIŞA AKTARIMA HAZIR",
      "orbit_mode": "Yörünge Modu",
      "developer": "Geliştirici",
      "alignment_settings": "KONUM AYARLARI",
      "auto_center": "Otomatik Ortala",
      "text_position": "Metin Konumu",
      "arc_radius": "KAVİS YARIÇAPI (R)",
      "base_height": "TABAN YÜKSEKLİĞİ",
      "border_settings": "KENARLIK AYARLARI",
      "border_enable": "Kenarlık Ekle",
      "border_width": "Kenarlık Kalınlığı (mm)",
      "border_depth": "Kenarlık Yüksekliği (mm)",
      "corner_radius": "Kenar Ovalliği (mm)",
      "hole_external": "Deliği Dışarıya Taşı",
      "text_settings": "YAZI AYARLARI",
      "base_settings": "TABAN & KENARLIK",
      "icon_settings": "SİMGE & DELİK",
      "color_settings": "RENK AYARLARI",
      "label_text_color": "Yazı Rengi",
      "label_base_color": "Taban Plakası Rengi",
      "fixed_length": "Sabit Genişlik",
      "width_cm": "cm",
      "auto": "OTO",
      "auto_length_tooltip": "Harf sayısına göre otomatik uzunluk",
      "text_scale": "Ana Yazı Boyutu (%)",
      "sub_text_scale": "Alt Yazı Boyutu (%)",
      "text_spacing": "Ana Yazı Harf Aralığı (mm)",
      "sub_text_spacing": "Alt Yazı Harf Aralığı (mm)",
      "text_alignment": "Yazı Hizalaması",
      "alignment_left": "Sola Dayalı",
      "alignment_center": "Ortalı",
      "alignment_right": "Sağa Dayalı",
      "main_sub_spacing": "Ana-Alt Metin Aralığı (%)",
      "export_single": "Tek Parça STL İndir",
      "export_multi": "Çift Renk (AMS) STL İndir",
      "ams_tip_title": "Çoklu Renk (AMS) Baskı",
      "ams_tip": "İndirdiğiniz ZIP dosyasındaki iki parçayı Bambu Studio'ya aynı anda sürükleyin. 'Tek obje olarak yüklensin mi?' sorusuna EVET deyin. Sol taraftaki Objeler (Objects) panelinden parçalara sağ tıklayıp farklı renk (flament) atayabilirsiniz.",
      "ignore_cantilever": "⚠️ Not: Bambu Studio'da 'Yüzen konsol var' (Floating cantilever) uyarısı alırsanız endişelenmeyin! Bu bir hata değildir; 'T' ve 'E' gibi yatay kollara sahip harflerin doğası gereği altının boş olmasından kaynaklanır. Uyarıyı yoksayıp basabilir veya dilerseniz 'Destek (Support)' açarak yazdırabilirsiniz.",
      "warning_font_turkish": "⚠️ Seçtiğiniz yazı tipi Türkçe karakterleri (ş, ğ, ı, vb.) tam desteklememektedir. Harflerin düzgün basılması ve görüntülenmesi için varsayılan 'Droid Sans' yazı tipini kullanmanızı öneririz.",
      "label_border_color": "Kenarlık Rengi",
      "special_char_e": "Futuristik E",
      "special_char_x": "Futuristik X",
      "special_char_heart": "Kalp",
      "svg_template": "📐 Şablon SVG İndir (Manuel Düzenle)"
    }
  },
  EN: {
    translation: {
      "title": "3D Keychain Generator",
      "settings_title": "Generator Settings",
      "settings_desc": "Customize your 3D printable keychain",
      "language": "Language",
      "label_text": "Main Text",
      "sub_text": "Sub Text (Optional)",
      "phone_text": "Back Side (Phone, etc.)",
      "placeholder": "Ex: YONCALI",
      "placeholder_sub": "Ex: MOBİLYA",
      "placeholder_phone": "Ex: +1 234 567 890",
      "hole_position": "Hole Position",
      "hole_top_left": "Top Left",
      "hole_center_left": "Center Left",
      "hole_bottom_left": "Bottom Left",
      "hole_top_right": "Top Right",
      "hole_center_right": "Center Right",
      "hole_bottom_right": "Bottom Right",
      "hole_none": "No Hole (Flat)",
      "hole_radius": "Hole Radius (mm)",
      "hole_thickness": "Loop Ring Thickness (mm)",
      "hole_height": "Loop Height (mm)",
      "text_style": "Text Style",
      "style_embossed": "Embossed",
      "style_engraved": "Engraved",
      "style_carved": "Carved",
      "warning_stencil_title": "Slicing & Print Warning:",
      "warning_stencil_desc": "The Writing Depth is equal to or greater than the Base Height. The text will completely pass through the base to create a cutout/stencil gap. If you want it to remain an embedded pocket, increase the base height or decrease the writing depth.",
      "base_shape": "Base Shape",
      "shape_rectangle": "Rectangle",
      "shape_teardrop": "Teardrop",
      "shape_heart": "Heart",
      "shape_contour": "Contour",
      "font_family": "Font Family",
      "icon": "Icon",
      "icon_none": "None",
      "icon_clover": "Clover",
      "icon_star_crescent": "Star Crescent",
      "icon_heart": "Heart",
      "icon_skull": "Skull",
      "icon_rook": "Rook",
      "icon_racket_table": "Table Tennis",
      "icon_racket_tennis": "Tennis Racket",
      "icon_i_love": "I ❤️ Format",
      "icon_custom": "Custom SVG",
      "icon_position": "Icon Position",
      "icon_pos_left": "Left",
      "icon_pos_top": "Top",
      "icon_pos_right": "Right",
      "transform": "Transform",
      "normal_text": "NORMAL",
      "italic": "ITALIC",
      "text_depth": "Embossing Height (mm)",
      "engrave_depth": "Engraving Depth (mm)",
      "phone_depth": "Back Engraving (mm)",
      "filament_color": "Filament Color",
      "plate_thickness": "PLATE THICKNESS",
      "tilt_angle": "TILT ANGLE",
      "export_btn": "EXPORT AS STL",
      "tip": "Tip: A tilt angle between 30-40 degrees ensures flawless FDM prints without the need for supports.",
      "export_ready": "READY FOR EXPORT",
      "orbit_mode": "Orbit Mode",
      "developer": "Developer",
      "alignment_settings": "ALIGNMENT SETTINGS",
      "auto_center": "Auto-Center",
      "text_position": "Text Position",
      "arc_radius": "ARC RADIUS (R)",
      "base_height": "BASE HEIGHT",
      "border_settings": "BORDER SETTINGS",
      "border_enable": "Add Border",
      "border_width": "Border Width (mm)",
      "border_depth": "Border Height (mm)",
      "corner_radius": "Corner Roundness (mm)",
      "hole_external": "Protrude Hole Outside",
      "text_settings": "TEXT SETTINGS",
      "base_settings": "BASE & BORDER",
      "icon_settings": "ICON & HOLE",
      "color_settings": "COLOR SETTINGS",
      "label_text_color": "Text Color",
      "label_base_color": "Base Plate Color",
      "fixed_length": "Fixed Width",
      "width_cm": "cm",
      "auto": "AUTO",
      "auto_length_tooltip": "Automatic length based on letter count",
      "text_scale": "Main Text Scale (%)",
      "sub_text_scale": "Sub Text Scale (%)",
      "text_spacing": "Main Text Letter Spacing (mm)",
      "sub_text_spacing": "Sub Text Letter Spacing (mm)",
      "text_alignment": "Text Alignment",
      "alignment_left": "Align Left",
      "alignment_center": "Center",
      "alignment_right": "Align Right",
      "main_sub_spacing": "Main-Sub Text Gap (%)",
      "export_single": "Download Single Part STL",
      "export_multi": "Download Dual-Color (AMS) ZIP",
      "ams_tip_title": "Multi-Color (AMS) Printing",
      "ams_tip": "Drag both pieces from the downloaded ZIP into Bambu Studio simultaneously. Click YES when asked 'Load as single object?'. You can assign different colors by right-clicking parts in the Objects panel on the left.",
      "ignore_cantilever": "⚠️ Note: If Bambu Studio shows a 'Floating cantilever' warning, don't panic! It's not a bug; it's the natural geometry of letters with horizontal arms (like 'T' or 'E'). You can safely ignore the warning or enable supports if preferred.",
      "warning_font_turkish": "⚠️ The selected font does not fully support Turkish characters (ş, ğ, ı, etc.). For perfect rendering and printing, we recommend using the default 'Droid Sans' font.",
      "label_border_color": "Border Color",
      "special_char_e": "Futuristic E",
      "special_char_x": "Futuristic X",
      "special_char_heart": "Heart",
      "svg_template": "📐 Download SVG Template (Edit Manually)"
    }
  },
  DE: {
    translation: {
      "title": "3D-Namensschild-Generator",
      "settings_title": "Generator-Einstellungen",
      "settings_desc": "Passen Sie Ihr 3D-Druck-Namensschild an",
      "language": "Sprache",
      "label_text": "Beschriftungstext",
      "sub_text": "Unterer Text (Optional)",
      "phone_text": "Rückseite (Telefon usw.)",
      "placeholder": "Bsp: 73",
      "placeholder_sub": "Bsp: MOBİLYA",
      "placeholder_phone": "Bsp: +49 151 2345678",
      "hole_position": "Lochposition",
      "hole_top_left": "Oben Links",
      "hole_center_left": "Mitte Links",
      "hole_bottom_left": "Unten Links",
      "hole_top_right": "Oben Rechts",
      "hole_center_right": "Mitte Rechts",
      "hole_bottom_right": "Unten Rechts",
      "hole_none": "Kein Loch",
      "hole_radius": "Lochradius (mm)",
      "hole_thickness": "Ringstärke (mm)",
      "hole_height": "Ringhöhe (mm)",
      "text_style": "Textstil",
      "style_embossed": "Erhaben",
      "style_engraved": "Eingraviert",
      "style_carved": "Geschnitzt",
      "warning_stencil_title": "Druck- & Slicing-Warnung:",
      "warning_stencil_desc": "Die Schrifttiefe ist größer oder gleich der Basishöhe. Der Text wird vollständig durch die Basis gehen und eine Aussparung/Schablone erzeugen. Wenn er als eingebettete Vertiefung bleiben soll, erhöhen Sie die Basishöhe oder verringern Sie die Schrifttiefe.",
      "base_shape": "Basisform",
      "shape_rectangle": "Rechteck",
      "shape_teardrop": "Tropfen",
      "shape_heart": "Herz",
      "shape_contour": "Kontur",
      "font_family": "Schriftart",
      "icon": "Symbol",
      "icon_none": "Keine",
      "icon_clover": "Kleeblatt",
      "icon_star_crescent": "Stern und Halbmond",
      "icon_heart": "Herz",
      "icon_skull": "Schädel",
      "icon_rook": "Turm",
      "icon_racket_table": "Tischtennis",
      "icon_racket_tennis": "Tennisschläger",
      "icon_i_love": "I ❤️ Format",
      "icon_custom": "Benutzerdefinierte SVG",
      "icon_position": "Symbolposition",
      "icon_pos_left": "Links",
      "icon_pos_top": "Oben",
      "icon_pos_right": "Rechts",
      "transform": "Transformieren",
      "normal_text": "NORMAL",
      "italic": "Kursiv",
      "text_depth": "Erhabenheitshöhe (mm)",
      "engrave_depth": "Gravurtiefe (mm)",
      "filament_color": "Filamentfarbe",
      "plate_thickness": "PLATTENDICKE",
      "tilt_angle": "NEIGUNGSWINKEL",
      "export_btn": "ALS STL EXPORTIEREN",
      "tip": "Tipp: Ein Neigungswinkel zwischen 30-40 Grad sorgt für fehlerfreie FDM-Drucke ohne Stützstrukturen.",
      "export_ready": "BEREIT ZUM EXPORT",
      "orbit_mode": "Orbit-Modus",
      "developer": "Entwickler",
      "arc_radius": "BOGENRADIUS (R)",
      "base_height": "BASISHÖHE",
      "border_settings": "RAND-EINSTELLUNGEN",
      "border_enable": "Rand Hinzufügen",
      "border_width": "Randbreite (mm)",
      "border_depth": "Randhöhe (mm)",
      "corner_radius": "Eckenrundung (mm)",
      "hole_external": "Loch nach außen verlegen",
      "label_text_color": "Textfarbe",
      "label_base_color": "Grundplattenfarbe",
      "fixed_length": "Feste Länge (Produktion)",
      "width_cm": "cm",
      "auto": "AUTO",
      "auto_length_tooltip": "Automatische Länge basierend auf Buchstabenanzahl",
      "text_scale": "Haupttextgröße (%)",
      "sub_text_scale": "Untertextgröße (%)",
      "text_spacing": "Buchstabenabstand Haupttext (mm)",
      "sub_text_spacing": "Buchstabenabstand Untertext (mm)",
      "text_alignment": "Textausrichtung",
      "alignment_left": "Linksbuendig",
      "alignment_center": "Zentriert",
      "alignment_right": "Rechtsbuendig",
      "main_sub_spacing": "Haupt-Untertext-Abstand (%)",
      "export_single": "Einzel-STL herunterladen",
      "export_multi": "Zweifarbige (AMS) ZIP herunterladen",
      "ams_tip_title": "Mehrfarbiger (AMS) Druck",
      "ams_tip": "Ziehen Sie beide Dateien aus der heruntergeladenen ZIP-Datei gleichzeitig in Bambu Studio. Klicken Sie auf JA, wenn Sie gefragt werden: 'Als einzelnes Objekt laden?'. Sie können verschiedene Farben zuweisen, indem Sie im Objektabschitt mit der rechten Maustaste klicken.",
      "ignore_cantilever": "⚠️ Hinweis: Keine Panik bei 'Floating cantilever' (schwebender Überhang) Warnungen! Dies ist kein Bug, sondern resultiert aus Buchstaben mit waagrechten Armen (wie 'T' oder 'E'). Ignorieren Sie die Warnung oder aktivieren Sie Stützstrukturen.",
      "warning_font_turkish": "⚠️ Die ausgewählte Schriftart unterstützt türkische Zeichen (ş, ğ, ı, etc.) nicht vollständig. Für eine korrekte Darstellung und Druck empfehlen wir die Standard-Schriftart 'Droid Sans'.",
      "label_border_color": "Randfarbe",
      "special_char_e": "Futuristisches E",
      "special_char_x": "Futuristisches X",
      "special_char_heart": "Herz",
      "svg_template": "📐 SVG-Vorlage herunterladen (Manuell bearbeiten)"
    }
  },
  AZ: {
    translation: {
      "title": "3D Adlıq İstehsalçısı",
      "settings_title": "Yaradıcı Ayarları",
      "settings_desc": "3D çap edilə bilən adlığınızı fərdiləşdirin",
      "language": "Dil",
      "label_text": "Etiket Mətni",
      "sub_text": "Alt Mətn (İstəyə bağlı)",
      "phone_text": "Arxa Üz (Telefon və s.)",
      "placeholder": "Məsələn: 73",
      "placeholder_sub": "Məsələn: MOBİLYA",
      "placeholder_phone": "Məsələn: +994 50 123 45 67",
      "hole_position": "Dəlik Mövqeyi",
      "hole_top_left": "Üst Sol",
      "hole_center_left": "Orta Sol",
      "hole_bottom_left": "Alt Sol",
      "hole_top_right": "Üst Sağ",
      "hole_center_right": "Orta Sağ",
      "hole_bottom_right": "Alt Sağ",
      "hole_none": "Dəliksiz",
      "hole_radius": "Dəlik Radiusu (mm)",
      "hole_thickness": "Həlqə Qalınlığı (mm)",
      "hole_height": "Həlqə Hündürlüyü (mm)",
      "text_style": "Yazı Üslubu",
      "style_embossed": "Qabarıq",
      "style_engraved": "Batıq (Gömülü)",
      "style_carved": "Oyulmuş",
      "warning_stencil_title": "Dəqiq Çap Xəbərdarlığı:",
      "warning_stencil_desc": "Yazı Dərinliyi, Baza Hündürlüyünə bərabər və ya ondan böyükdür. Yazı brelokun içindən tamamilə keçərək boşluq yaradacaqdır. Batıq (gömülü) qalması üçün baza hündürlüyünü artıra və ya yazı dərinliyini azalda bilərsiniz.",
      "base_shape": "Baza Şəkli",
      "shape_rectangle": "Düzbucaqlı",
      "shape_teardrop": "Damcı",
      "shape_heart": "Ürək",
      "shape_contour": "Kontur",
      "font_family": "Yazı Tipi",
      "icon": "Simge",
      "icon_none": "Yox",
      "icon_clover": "Yonca",
      "icon_star_crescent": "Ay Ulduz",
      "icon_heart": "Ürək",
      "icon_skull": "Kəllə",
      "icon_rook": "Qala",
      "icon_racket_table": "Stolüstü tennis",
      "icon_racket_tennis": "Tennis raketi",
      "icon_i_love": "I ❤️ Formatı",
      "icon_custom": "Xüsusi SVG",
      "icon_position": "Simge Mövqeyi",
      "icon_pos_left": "Sol",
      "icon_pos_top": "Üst",
      "icon_pos_right": "Sağ",
      "transform": "Çevirmə",
      "normal_text": "NORMAL",
      "italic": "İtalik",
      "text_depth": "Qabarıqlıq Hündürlüyü (mm)",
      "engrave_depth": "Batıq Dərinliyi (mm)",
      "filament_color": "Filament Rəngi",
      "plate_thickness": "LÖVHƏ QALINLIĞI",
      "tilt_angle": "ƏYİLMƏ BUCAĞI",
      "export_btn": "STL FAYLI KİMİ ÇIXAR",
      "tip": "İpucu: 30-40 dərəcəlik bir əyilmə, dəstəyə ehtiyac olmadan qüsursuz FDM çapını təmin edir.",
      "export_ready": "İXRACA HAZIRDIR",
      "orbit_mode": "Orbit Rejimi",
      "developer": "Tərtibatçı",
      "arc_radius": "QÖVS RADİUSU (R)",
      "base_height": "BAZA HÜNDÜRLÜYü",
      "border_settings": "KƏNARLIQ AYARLARI",
      "border_enable": "Kənarlıq Əlavə Et",
      "border_width": "Kənarlıq Qalınlığı (mm)",
      "border_depth": "Kənarlıq Hündürlüyü (mm)",
      "corner_radius": "Künc Yuvarlaqlığı (mm)",
      "hole_external": "Dəliyi Kənara Çıxart",
      "label_text_color": "Yazı Rəngi",
      "label_base_color": "Baza Lövhəsi Rəngi",
      "fixed_length": "Sabit Uzunluq (İstehsal)",
      "width_cm": "sm",
      "auto": "AVTO",
      "auto_length_tooltip": "Hərf sayına görə avtomatik uzunluq",
      "text_scale": "Ana Yazı Ölçüsü (%)",
      "sub_text_scale": "Alt Yazı Ölçüsü (%)",
      "text_spacing": "Ana Yazı Hərf Aralığı (mm)",
      "sub_text_spacing": "Alt Yazı Hərf Aralığı (mm)",
      "text_alignment": "Yazı Düzləndirilməsi",
      "alignment_left": "Sola Söykənmiş",
      "alignment_center": "Mərkəzləşdirilmiş",
      "alignment_right": "Sağa Söykənmiş",
      "main_sub_spacing": "Ana-Alt Mətn Aralığı (%)",
      "export_single": "Tək Parça STL Yüklə",
      "export_multi": "Çox Rəngli (AMS) ZIP Yüklə",
      "ams_tip_title": "Çox Rəngli (AMS) Çap",
      "ams_tip": "Yüklədiyiniz ZIP faylındakı iki parçanı eyni anda Bambu Studio-ya sürükləyin. 'Tək obyekt kimi yüklənsin?' sualına BƏLİ deyin. Sol tərəfdəki Obyektlər panelindən hissələrə sağ klikləməklə müxtəlif rənglər (filament) təyin edə bilərsiniz.",
      "ignore_cantilever": "⚠️ Qeyd: Bambu Studio 'Yüzen konsol var' (Floating cantilever) xəbərdarlığını verərsə narahat olmayın! Bu səhv deyil; 'T' ve 'E' kimi qolları olan hərflərin havada qalma təbiətidir. Xəbərdarlığa məhəl qoymadan və ya 'Dəstək' (Support) açaraq çap edə bilərsiniz.",
      "warning_font_turkish": "⚠️ Seçdiyiniz şrift Azərbaycan/Türk hərflərini (ş, ğ, ı, vb.) tam dəstəkləmir. Düzgün çap və görünüş üçün standart 'Droid Sans' şriftindən istifadə etməyinizi tövsiyə edirik.",
      "label_border_color": "Kənarlıq Rəngi",
      "special_char_e": "Futuristik E",
      "special_char_x": "Futuristik X",
      "special_char_heart": "Ürək",
      "svg_template": "📐 Şablon SVG Yüklə (Əl ilə Düzəliş Et)"
    }
  },
  ES: {
    translation: {
      "title": "Generador de Placas de Nombre 3D",
      "settings_title": "Ajustes del Generador",
      "settings_desc": "Personaliza tu placa de nombre imprimible en 3D",
      "language": "Idioma",
      "label_text": "Texto de la Etiqueta",
      "sub_text": "Subtexto (Opcional)",
      "phone_text": "Lado Posterior (Teléfono, etc.)",
      "placeholder": "Ej: 73",
      "placeholder_sub": "Ej: MOBİLYA",
      "placeholder_phone": "Ej: +34 600 123 456",
      "hole_position": "Posición del agujero",
      "hole_top_left": "Arriba Izquierda",
      "hole_center_left": "Centro Izquierda",
      "hole_bottom_left": "Abajo Izquierda",
      "hole_top_right": "Arriba Derecha",
      "hole_center_right": "Centro Derecha",
      "hole_bottom_right": "Abajo Derecha",
      "hole_none": "Sin Agujero",
      "hole_radius": "Radio del Agujero (mm)",
      "hole_thickness": "Grosor del Anillo (mm)",
      "hole_height": "Altura del Anillo (mm)",
      "text_style": "Estilo de Texto",
      "style_embossed": "Relieve",
      "style_engraved": "Grabado",
      "style_carved": "Tallado",
      "warning_stencil_title": "Advertencia de Impresión:",
      "warning_stencil_desc": "La Profundidad del Texto es igual o mayor que la Altura de la Base. El texto pasará completamente a través de la base para crear un hueco/plantilla. Si desea que permanezca como un grabado incrustado, aumente la altura de la base o disminuya la profundidad del texto.",
      "base_shape": "Forma de la Base",
      "shape_rectangle": "Rectángulo",
      "shape_teardrop": "Lágrima",
      "shape_heart": "Corazón",
      "shape_contour": "Contorno",
      "font_family": "Tipo de Letra",
      "icon": "Icono",
      "icon_none": "Ninguno",
      "icon_clover": "Trébol",
      "icon_star_crescent": "Estrella y Media Luna",
      "icon_heart": "Corazón",
      "icon_skull": "Calavera",
      "icon_rook": "Torre",
      "icon_racket_table": "Tenis de Mesa",
      "icon_racket_tennis": "Raqueta de Tenis",
      "icon_i_love": "Formato I ❤️",
      "icon_custom": "SVG Personalizado",
      "icon_position": "Posición del Icono",
      "icon_pos_left": "Izquierda",
      "icon_pos_top": "Arriba",
      "icon_pos_right": "Derecha",
      "transform": "Transformar",
      "normal_text": "NORMAL",
      "italic": "Cursiva",
      "text_depth": "Altura del Relieve (mm)",
      "engrave_depth": "Profundidad de Grabado (mm)",
      "filament_color": "Color del Filamento",
      "tilt_angle": "ÁNGULO DE INCLINACIÓN",
      "export_btn": "EXPORTAR COMO STL",
      "tip": "Consejo: Un ángulo de inclinación entre 30-40 grados garantiza impresiones FDM perfectas sin necesidad de soportes.",
      "export_ready": "LISTO PARA EXPORTAR",
      "orbit_mode": "Modo Órbita",
      "developer": "Desarrollador",
      "arc_radius": "RADIO DEL ARCO (R)",
      "base_height": "ALTURA DE LA BASE",
      "border_settings": "AJUSTES DEL BORDE",
      "border_enable": "Añadir Borde",
      "border_width": "Ancho del Borde (mm)",
      "border_depth": "Altura del Borde (mm)",
      "corner_radius": "Redondez de Esquinas (mm)",
      "hole_external": "Desplazar Agujero Hacia Afuera",
      "label_text_color": "Color del Texto",
      "label_base_color": "Color de la Placa Base",
      "fixed_length": "Longitud Fija (Producción)",
      "width_cm": "cm",
      "auto": "AUTO",
      "auto_length_tooltip": "Longitud automática basada en el número de letras",
      "text_scale": "Escala de Texto Principal (%)",
      "sub_text_scale": "Escala de Subtexto (%)",
      "text_spacing": "Espaciado de Letras Principal (mm)",
      "sub_text_spacing": "Espaciado de Letras Secundario (mm)",
      "text_alignment": "Alineación de Texto",
      "alignment_left": "Alinear a la Izquierda",
      "alignment_center": "Centrar",
      "alignment_right": "Alinear a la Derecha",
      "main_sub_spacing": "Espaciado Principal-Sub (%)",
      "export_single": "Descargar STL de Pieza Única",
      "export_multi": "Descargar ZIP de Doble Color (AMS)",
      "ams_tip_title": "Impresión Multi-color (AMS)",
      "ams_tip": "Arrastra ambas piezas desde el ZIP descargado a Bambu Studio simultáneamente. Haz clic en SÍ cuando se te pregunte '¿Cargar como un solo objeto?'. Puedes asignar diferentes colores haciendo clic derecho en las piezas en el panel de Objetos a la izquierda.",
      "ignore_cantilever": "⚠️ Nota: Si Bambu Studio muestra un aviso de 'Voladizo flotante' (Floating cantilever), ¡no entres en pánico! No es un error; es la geometría natural de las letras con brazos horizontales (como 'T' o 'E'). Puedes ignorar el aviso con seguridad o activar soportes si lo prefieres.",
      "warning_font_turkish": "⚠️ La tipografía seleccionada no es totalmente compatible con caracteres turcos (ş, ğ, ı, etc.). Recomendamos usar la fuente predeterminada 'Droid Sans' para una visualización e impresión perfectas.",
      "label_border_color": "Color del Borde",
      "special_char_e": "E Futurista",
      "special_char_x": "X Futurista",
      "special_char_heart": "Corazón",
      "svg_template": "📐 Descargar Plantilla SVG (Editar Manualmente)"
    }
  }
};

i18n
  .use(initReactI18next)
  .init({
    resources,
    lng: "TR", 
    fallbackLng: "EN",
    interpolation: {
      escapeValue: false 
    }
  });

export default i18n;
